...
provision: rebuild-docker install build-proto-schema build build-libs migrate dump-schema
...
build-proto-schema:
	${DOCKER_COMPOSE_RUN} -e "NODE_ENV=${NODE_ENV}" app ./gen-proto-schema.sh

build-libs:
	${DOCKER_COMPOSE_RUN} -e "NODE_ENV=${NODE_ENV}" app nest build proto-schema
...
psql:
	${DOCKER_COMPOSE_RUN} app psql -h db -U postgres invest_${NODE_ENV}
...
dump-schema:
	${DOCKER_COMPOSE_RUN} app pg_dump -h db -U postgres ${PROJECT_NAME}_${NODE_ENV} -s -x -O -f db/schema.sql

migration-generate:
	${DOCKER_COMPOSE_RUN} -e "NODE_ENV=${NODE_ENV}" app yarn typeorm migration:generate -p -d ./db/data-source.ts ./db/migrations/$(name)

migration-create:
	${DOCKER_COMPOSE_RUN} -e "NODE_ENV=${NODE_ENV}" app yarn typeorm migration:create ./db/migrations/$(name)
